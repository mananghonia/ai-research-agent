...
const mdComponents = {
  ...
  // Code: inline vs block vs mermaid
  // In react-markdown v10, inline code has no newlines and no language class
  code: ({ className, children }) => {
    ...
    if (lang === 'mermaid') {
      return <MermaidChart code={raw.trim()} />
    }
  ...
  h1: ({ children }) => (
    <h1 className="text-2xl font-bold text-slate-100 mt-8 mb-4 first:mt-0">{children}</h1>
  ),
  h2: ({ children }) => (
    <h2 className="text-lg font-semibold text-slate-100 mt-7 mb-3 pb-2 border-b border-slate-700/60">{children}</h2>
  ),
  h3: ({ children }) => (
    <h3 className="text-base font-semibold text-slate-200 mt-5 mb-2">{children}</h3>
  ),
  h4: ({ children }) => (
    <h4 className="text-sm font-semibold text-slate-300 mt-4 mb-1.5 uppercase tracking-wide">{children}</h4>
  ),

  ul: ({ children }) => (
    <ul className="my-3 space-y-1.5 pl-5 list-disc marker:text-violet-500">{children}</ul>
  ),
  ol: ({ children }) => (
    <ol className="my-3 space-y-1.5 pl-5 list-decimal marker:text-violet-500">{children}</ol>
  ),
  li: ({ children }) => (
    ...
    <div className="w-full space-y-6 overflow-hidden">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex gap-2 shrink-0">
          <a href={getPdfUrl(sessionId)} download
